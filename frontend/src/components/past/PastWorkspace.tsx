  ...
  return (
    <div className="max-w-7xl mx-auto px-3 sm:px-6 py-4 sm:py-6 space-y-6">
      <div className="bg-white border border-[#DDDDDD] rounded-2xl p-5 sm:p-6 shadow-sm space-y-5">
        ...
        {/* Hazard filter chips with icons */}
        <div className="flex flex-wrap items-center gap-2 pt-1 border-t border-[#DDDDDD]">
          {HAZARD_FILTERS.map((filter) => {
            const Icon = filter.icon;
            const active = hazardFilter === filter.key;
            return (
              <button
                key={filter.key}
                type="button"
                onClick={() => setHazardFilter(filter.key)}
                className={`px-3 py-1.5 rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-all cursor-pointer ${
                  active
                    ? 'bg-[#0F1B29] text-white shadow-xs'
                    : 'bg-[#ECF8F8]/60 hover:bg-[#ECF8F8] text-[#0F1B29] border border-[#DDDDDD]'
                }`}
              >
        ...
        {/* Secondary filters: state, decade, and sort */}
        <div className="flex flex-wrap items-center gap-3 pt-2">
          <div className="flex items-center gap-1.5 text-xs text-[#747F8D]">
          ...
          <select
            ...
            className="px-3 py-1.5 rounded-xl bg-white border border-[#DDDDDD] text-xs font-medium text-[#0F1B29] focus:outline-none focus:border-[#747F8D]"
          >
          ...
          <select
            value={eraFilter}
            onChange={(e) => setEraFilter(e.target.value as (typeof ERA_FILTERS)[number])}
            className="px-3 py-1.5 rounded-xl bg-white border border-[#DDDDDD] text-xs font-medium text-[#0F1B29] focus:outline-none focus:border-[#747F8D]"
          >
            {ERA_FILTERS.map((era) => (
          ...
          <div className="ml-auto flex items-center gap-2">
            <span className="text-xs text-[#747F8D]">Sort by:</span>
            <select
              value={sortOrder}
            ...
            >
              ...
              <option value="sources">Most Sources Cited</option>
