...
import { alertEnumLabel, UI_TEXT, uiText } from '../../lib/uiText';
...
export const AlertDetailDrawer: React.FC<AlertDetailDrawerProps> = ({
  ...
  const sourceText = (_id: string, text: string) => text;
        ...
        {activeTab === 'info' && (
          <div className="space-y-4 animate-in fade-in">
            ...
            <div className="space-y-1.5">
              <h4 className="font-bold text-sm sm:text-base text-slate-900">{sourceText('headline', alert.headline)}</h4>
              <p className="text-xs sm:text-sm text-slate-600 leading-relaxed">{sourceText('description', alert.description)}</p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              ...
              {alert.polygon && alert.polygon.coordinates && (
                <div className="p-3 rounded-xl bg-slate-50 border border-slate-200 col-span-2">
                  <span className="text-slate-500 text-[10px] uppercase font-bold">{uiText('alerts.coordinates', { count: alert.polygon.coordinates.length })}</span>
                  ...
                  <div className="space-y-2">
                    {guidance.actionableMeasures.map((measure, idx) => (
                      ...
                      >
                        ...
                        <span>{sourceText(`measure.${idx}`, measure)}</span>
                      </div>
                    ))}
                  </div>
                </div>
                  ...
                  <div className="p-3 rounded-xl bg-emerald-50 border border-emerald-200 space-y-1.5">
                    ...
                    {guidance.dos.map((d, idx) => (
                      <div key={idx} className="text-[11px] text-slate-700 leading-snug flex items-start gap-1">
                        ...
                        <span>{sourceText(`dos.${idx}`, d)}</span>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="p-3 rounded-xl bg-rose-50 border border-rose-200 space-y-1.5">
                    ...
                    {guidance.donts.map((d, idx) => (
                      <div key={idx} className="text-[11px] text-slate-700 leading-snug flex items-start gap-1">
                        ...
                        <span>{sourceText(`donts.${idx}`, d)}</span>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          <div className="space-y-3 animate-in fade-in">
            ...
            <div className="space-y-2">
              {guidance.emergencyContacts.map((c, idx) => (
                ...
                >
                  <div>
                    <div className="text-xs font-bold text-slate-900">{sourceText(`contact.${idx}.label`, c.label)}</div>
                    <div className="text-[11px] text-slate-500">{sourceText(`contact.${idx}.description`, c.description)}</div>
                  </div>
