    ...
    <div className="relative inline-flex items-center">
      {isRecording ? (
        <div className="flex items-center gap-2 px-2.5 py-1.5 rounded-xl bg-rose-50 border border-rose-200 shadow-sm">
          ...
          <button type="button" onClick={stopRecording} className={`flex items-center gap-1 px-2 py-1 rounded-lg bg-rose-600 hover:bg-rose-700 text-white font-bold text-xs transition-all ${className}`} title={uiText('voice.stop', { seconds: recordingDuration })}>
            <Square className="w-3 h-3 fill-current" />
            <span>{uiText('voice.stop', { seconds: recordingDuration })}</span>
          </button>
        </div>
      ) : isTranscribing ? (
        <button
