...
export const AudioRecorderButton: React.FC<AudioRecorderButtonProps> = ({
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      setErrorMessage(uiText('voice.microphoneDenied'));
  ...
  const handleTranscribe = async (blob: Blob, mimeType: string) => {
    try {
      ...
      if (data.text) {
        ...
        setErrorMessage(uiText('voice.noSpeech'));
      }
    } catch (err) {
      console.error('Transcription API error:', err);
      setErrorMessage(uiText('voice.transcriptionError'));
    } finally {
      setIsTranscribing(false);
    }
  };
      ...
      {isRecording ? (
        <div className="flex items-center gap-2 px-2.5 py-1.5 rounded-xl bg-rose-50 border border-rose-200 shadow-sm">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-rose-600 hover:bg-rose-700 active:bg-rose-800 text-white font-bold text-xs transition-all shadow-xs cursor-pointer"
            title={uiText('voice.stop', { seconds: recordingDuration })}
          >
            <Square className="w-3 h-3 fill-white text-white" />
            <span className="text-white font-bold">{uiText('voice.stop', { seconds: recordingDuration })}</span>
          </button>
        </div>
      ) : isTranscribing ? (
        <button
        ...
        >
          ...
          <span>{uiText('voice.transcribing')}</span>
        </button>
      ) : (
        <button
          type="button"
          ...
          title={tooltip || uiText('voice.record')}
        >
