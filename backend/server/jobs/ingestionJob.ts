...
async function storeObservation(obs: NormalizedObservation): Promise<string | null> {
  ...
  try {
    // Upsert by the concrete unique constraint available to PostgREST. The
    // migration also has a partial (source_id, external_id) index, but partial
    // indexes cannot be selected reliably with on_conflict.
