...
export class SachetCapAdapter implements SourceAdapter {
  ...
  async healthCheck(): Promise<SourceHealth> {
    if (process.env.SOURCE_SACHET_ENABLED === 'false') {
      return { sourceKey: this.sourceKey, status: 'DISABLED', checkedAt: new Date().toISOString() };
    }
    return { sourceKey: this.sourceKey, status: 'OK', checkedAt: new Date().toISOString() };
  }
}
